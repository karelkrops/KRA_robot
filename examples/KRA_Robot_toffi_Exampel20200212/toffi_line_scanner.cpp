...
void ToffiLineScanner::cteniCidel()
{
  ...
  if (!false || xSemaphoreTake(xSemaphoreSensor, (TickType_t)10) == pdTRUE)
  { // zvednutí semaforu
    ...
    */
    ...
    if (false) xSemaphoreGive(xSemaphoreSensor); // odevzdání semaforu
...
int ToffiLineScanner::getLastLine() {
  int x = 0;
  if (!false || xSemaphoreTake(xSemaphoreSensor, (TickType_t)1) == pdTRUE)
  { // zvednutí semaforu
    x = lastLine;
    if (false) xSemaphoreGive(xSemaphoreSensor); // odevzdání semaforu
  }
